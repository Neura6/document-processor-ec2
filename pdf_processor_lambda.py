...
def handle_upload(files):
    ...
    for file_path in files:
        ...
        if file_path.endswith('.pdf'):
            ...
            all_files_to_check.extend(folder_files)

    # Check if files exist in the source bucket (instead of processing)
    existing_files = []
    missing_files = []
    
    for file_path in all_files_to_check:
        try:
            s3.head_object(Bucket=SOURCE_BUCKET, Key=file_path)
            existing_files.append(file_path)
            logger.info(f"File exists in rules-repository: {file_path}")
        except s3.exceptions.ClientError as e:
            if e.response['Error']['Code'] == '404':
                missing_files.append(file_path)
                logger.warning(f"File not found in rules-repository: {file_path}")
            else:
                logger.error(f"Error checking file {file_path}: {str(e)}")
                missing_files.append(file_path)

    # Commented out: Process all files in parallel (chunking)
    # try:
    ...
    # Commented out: Sync to knowledge base for each root folder, handling failed files by moving
    # sync_results = {}
    ...
    if missing_files:
        return create_response(200, 'Partial Success', 
                             f'Found {len(existing_files)} files uploaded successfully. Missing files: {missing_files}. Uploaded files: {existing_files}')

    return create_response(200, 'Success', f'All {len(existing_files)} files have been uploaded successfully to rules-repository. Files: {existing_files}')
...
def handle_delete(folders):
    ...
    for folder in folders:
        # Commented out folder validation
        ...
        try:
            folder_files = list_files_in_folder(SOURCE_BUCKET, folder)
            
            for file_key in folder_files:
                if delete_s3_object(SOURCE_BUCKET, file_key):
            ...
            failed_deletes.append(f"{folder} (folder processing error)")

        # Commented out: Start a deletion sync for the folder
        # try:
